  ...
  return (
    <>
      ...
      >
        ...
        >
          {clusterSize.toLocaleString()} txs (
          {pct < 0.1 ? "<0.1%" : `${pct.toFixed(1)}%`} of network)
        </Badge>
      </Flex>
      <p
        style={{
...
export function Outliers() {
  ...
  const { data: detail, isLoading } = useClusterDetail(
    selectedNetwork,
    latestRunId,
  );
  const { data: outlierData } = useOutliers(
    selectedNetwork,
    latestRunId,
    10000,
  );
  ...
  return (
    <PageContainer>
      ...
      >
        Each cluster represents a privacy set — a group of transactions that
        look similar to each other. Larger clusters provide better privacy.
        Click a cluster to see its member transactions.
      </p>

      {!detail?.clusterSizes?.length ? (
        <Card>
          <p style={{ color: theme.colors.textMuted }}>
            No analysis data yet. Analysis runs automatically every 10 minutes.
          </p>
        </Card>
      ) : (
        <>
              ...
              <Table>
                <thead>
                  <tr>
                    ...
                    <th>% of Network</th>
                    <SortableHeader
                      $active={sortKey === "avgOutlierScore"}
                      onClick={() => handleSort("avgOutlierScore")}
                    >
                      Outlier Scores (avg / max within)
                      {sortIndicator("avgOutlierScore")}
                    </SortableHeader>
                    <th>Risk</th>
                  </tr>
                </thead>
                  ...
                  {paged.map((c) => {
                    ...
                    return (
                      <tr
                        ...
                        onClick={() =>
                          setSearchParams({ cluster: String(c.clusterId) })
                        }
                      >
                        <td>
                          {isOutlier ? (
                            <Badge color={theme.colors.outlier}>
                              Outliers ({size.toLocaleString()} txs)
                            </Badge>
                          ) : (
                            <Badge
                              color={
                                theme.colors.cluster[
                                  c.clusterId % theme.colors.cluster.length
                                ]
                              }
                            >
                              Cluster {c.clusterId}
                        ...
                        <td>
                          ...
                          >
                            {isOutlier
                              ? `1 each (${size.toLocaleString()} txs)`
                              : size.toLocaleString()}
                          </Badge>
                        </td>
                        <td
                        ...
                        <td>
                          {isOutlier ? (
                            ...
                            <span>
                              {c.avgOutlierScore != null
                                ? `${(c.avgOutlierScore * 100).toFixed(1)}%`
                                : "0%"}
                              {" / "}
                              <span
                                style={{
                                  color:
                                    (c.maxOutlierScore ?? 0) > 0.3
                                      ? theme.colors.warning
                                      : theme.colors.textMuted,
                                }}
                              >
                                {c.maxOutlierScore != null
                                  ? `${(c.maxOutlierScore * 100).toFixed(1)}%`
                                  : "0%"}
                              </span>
                            </span>
                          )}
