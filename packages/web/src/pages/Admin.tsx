...
import { useLogin, useTriggerAnalysis, useSaveAnalysisConfig, useRevertAnalysisConfig, useAnalysisStatus } from "../api/hooks";
...
export function Admin() {
  ...
  return (
    <PageContainer>
      ...
      <Card>
        <Flex gap="24px" style={{ flexWrap: "wrap", marginBottom: theme.spacing.md }}>
          <Label>
            Min cluster size
            <ParamInput
              type="number"
        ...
        <Flex gap="12px" align="center" style={{ flexWrap: "wrap" }}>
          ...
          {(saveConfig.isError || revertConfig.isError) && (
            <ErrorMsg>{(saveConfig.error ?? revertConfig.error)?.message}</ErrorMsg>
          )}
          {trigger.isError && (
            <ErrorMsg>{trigger.error?.message}</ErrorMsg>
          )}
        </Flex>
        ...
        {trigger.isError && trigger.error instanceof ApiError && (trigger.error.detail?.stderr || trigger.error.detail?.stdout) && (
          <div style={{ marginTop: theme.spacing.md }}>
            <OutputPre>
              {[trigger.error.detail.stderr, trigger.error.detail.stdout].filter(Boolean).join("\n")}
            </OutputPre>
          </div>
        )}
      </Card>
