...
async function main() {
  ...
  const log = createLogger(`indexer:${config.id}`);

  if (config.enabled === false) {
    log.info("Network is disabled. Exiting.");
    process.exit(0);
  }

  log.info(`Connecting to node at ${config.nodeUrl}...`);
  const node = createAztecNodeClient(config.nodeUrl);

  // Verify connectivity
  try {
    const blockNumber = await node.getBlockNumber();
    log.info(`Connected. Current block: ${blockNumber}`);
  } catch (err) {
  ...
  if (existing.length === 0) {
    ...
    log.info("Network registered in DB.");
  }

  // 1. Reconcile non-finalized txs on startup + periodically (every 60s)
  log.info("Starting reconciler...");
  await startReconciler(config.id, node, db, log.createChild("reconciler"));
  ...
  log.info(
    `Block stream starting from block ${startingBlock} ` +
    `(cursor: ${cursorBlock}, node tip: ${currentNodeBlock})`
  );

  const blockProcessor = new BlockProcessor(config.id, node, db, log.createChild("blocks"));
  ...
  log.info("Indexer running (mempool watcher + block stream).");
