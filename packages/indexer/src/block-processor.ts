    ...
    for (const block of event.blocks) {
      ...
      await this.db
        ...
        .values({
          ...
          totalFees: block.header.totalFees.toBigInt().toString(),
          totalManaUsed: block.header.totalManaUsed.toBigInt().toString(),
      ...
      for (let i = 0; i < block.body.txEffects.length; i++) {
        ...
        const vector = computeFeatureVector({
          ...
          hasTeardown: existing?.hasTeardown ?? (pf?.hasTeardown as unknown as boolean) ?? false,
